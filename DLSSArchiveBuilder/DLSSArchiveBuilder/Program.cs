...
// In debug mode this will be up one directory into the same directory the sln exists in.
string workingDataDirectory = Path.Combine(Directory.GetCurrentDirectory(), "WorkingData");
if (Directory.Exists(workingDataDirectory) == false)
{
    Directory.CreateDirectory(workingDataDirectory);
}

string outputDirectory = Path.Combine(workingDataDirectory, "output");
string baseDllDirectory = Path.Combine(workingDataDirectory, "base_dlss");
...
Parallel.ForEach(dlssRecrods.Stable, (dlssRecord) =>
{
    Console.WriteLine("Checking " + dlssRecord.DownloadUrl);
...
Parallel.ForEach(dlssRecrods.Experimental, (dlssRecord) =>
{
    var expectedPath = Path.Combine(baseDllDirectory, Path.GetFileName(dlssRecord.DownloadUrl));
    if (File.Exists(expectedPath))
    {
...
{
    // No parallel, do this one at a time to keep server happy.
    foreach (var item in dlssToDownload)
    {
...
File.WriteAllText(Path.Combine(workingDataDirectory, "dlss_records.json"), json);
File.WriteAllText(Path.Combine(workingDataDirectory, "additional_notes.txt"), additionalNotes.ToString());
...
void CreateZip(DLSSRecord dlssRecord, string outputZipName)
{
